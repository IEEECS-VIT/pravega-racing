...
const articles = [
  ...
  {
    title: "Vellore Engineering Students Ready to Test Their Formula Car in Germany",
    source: "NEWS 18",
            ...
            <div className={styles.articleCard} key={index}>
              <a href={article.link} target="_blank" rel="noopener noreferrer">
                <div className={styles.overlay}>
                  ...
                  <div className={`${styles.overlayContent} ${styles.textCenter}`}>
                    <span className={styles.overlayHeading}>{article.title}</span>
                    {article.source && <span className={styles.overlayText}><b>{article.source}</b></span>}
                  </div>
                  <img src={article.image} alt={article.title} className={`${styles.overlayImage} ${styles.teamPhoto}`} />
                </div>
              </a>
            </div>
          ))}
