        ...
        {chunkedMembers.map((row, rowIndex) => (
          ...
          >
            {row.map((member, memberIndex) => (
              <div
                key={memberIndex}
                className='relative overflow-hidden group gap-5 flex px-4 flex-col items-center justify-start w-[200px] h-64'
              >
            ...
            {rowIndex === chunkedMembers.length - 1 &&
              [...Array(6 - row.length)].map((_, index) => (
                <div
                  key={`empty-${index}`}
                  className='flex px-7 flex-col w-[200px] items-center justify-between h-64'
                >
